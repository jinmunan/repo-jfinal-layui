package com.cj.module.core.generator.controller;

import com.cj.common.base.controller.BaseController;
import com.cj.module.core.generator.service.CodeService;
...
public class CodeController extends BaseController {
    ...
    public void createCode() {
        ...
        for (int i = 0; i < tableNameArray.length; i++) {
            ...
            // 设置模板地址
            // 创建基本model模板
            Record codeRecord = codeService.createCodeTemplate(record, formItem, tableItem);
            codeList.add(codeRecord);
            modelList.add(modelPackage + modelNameArray[i]);
        }

        // 存储数据，用于创建本地文件,存储到session域中
    ...
    private Record subRecord(Record record, StringBuffer subTableTitle, List<Record> codeList, List<String> modelList) {
        ...
        subTableTitle.append("'")
                .append("".equals(tableComment) ? "Tab" : tableComment)
                .append("'");
